...
func colTy(md *opt.Metadata, col opt.ColumnID) string {
...
func joinKind(op opt.Operator) string {
	...
	case opt.AntiJoinOp:
		return "ANTI"
	default:
		return fmt.Sprintf("?not-implemented (%v)?", op.String())
	}
}
...
func (e Env) RelNode(rel RelExpr) (any, opt.ColList) {
	...
	case *ValuesExpr:
		...
		for _, col := range rel.Cols {
			schema = append(schema, colTy(md, col))
		}
	...
	case *GroupByExpr:
		...
		se := e.extend(sourceScope)
		groupProj, groupScope := e.remap(md, rel.GroupingCols, source, sourceScope)
		matchList := arr{}
		for idx, col := range groupScope {
			matchIdx := int(se.col(col)) + len(groupScope)
	...
	case *SortExpr:
		input, inputScope := e.RelNode(rel)
		sortEnv := e.extend(inputScope)
		collationList := arr{}
		for _, cr := range rel.InputOrdering.Columns {
			r := "ASCENDING"
			if cr.Descending {
				r = "DESCENDING"
			}
	...
	case *LimitExpr:
		...
		sortEnv := e.extend(inputScope)
		collationList := arr{}
		for _, cr := range rel.Ordering.Columns {
			r := "ASCENDING"
			if cr.Descending {
				r = "DESCENDING"
			}
